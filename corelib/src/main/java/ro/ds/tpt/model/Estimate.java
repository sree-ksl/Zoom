...
public class Estimate implements Serializable {
	...
	private ro.ds.tpt.model.EstimateType type;
	...
	public Estimate(Path path, Station station, int stationIndex, HourlyPlan plan) {
		...
		this.times1 = EMPTY;
		this.times2 = EMPTY;
		this.type = ro.ds.tpt.model.EstimateType.None;
		this.plan = plan;
	}
	...
	public int updateStation(IPrefs prefs, int ec) {
		try {
			if(ec<3) {
				...
				String[] t = ro.ds.tpt.RATT.downloadTimes(prefs, extLineId, extStationId);
	...
	public void putTime(String t1, String t2, String timestamp) {
		...
		if (t1!=null && t1.trim().length()==5 && t1.charAt(2)==':')
			this.type = Formatting.isInteger(t1.substring(0,2)) && Formatting.isInteger(t1.substring(3, 5))
					? ro.ds.tpt.model.EstimateType.Scheduled
					: ro.ds.tpt.model.EstimateType.None;
		else if(">>".equals(times1) || Formatting.isInteger(times1))
			this.type = ro.ds.tpt.model.EstimateType.GPS;
		else if(Formatting.isMinutes(times1)) {
			this.type = ro.ds.tpt.model.EstimateType.GPS;
			this.times1 = Formatting.parseMinutes(times1);
		} else
			this.type = ro.ds.tpt.model.EstimateType.None;
	}
	...
	public ro.ds.tpt.model.EstimateType getEstimateType() {
		return type;
	}
	
	String getTimes1() {
		return Formatting.formatTime(times1);
	}
	String getTimes2() {
		return Formatting.formatTime(times2);
	}
