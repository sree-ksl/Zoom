...
public class CityTest {
	...
	private HashMap<String, ro.ds.tpt.model.Station> stationExtIdMap;

	@Before
	public void setUp() throws IOException {
		c = ro.ds.tpt.JavaCityLoader.loadCachedCityOrDownloadAndCache(new TestPrefs());

		app_lines = new ArrayList<String>();
		for(LineKind kind : LineKind.values())
			app_lines.addAll(Arrays.asList(kind.getLineNames()));
		
		stationExtIdMap = new HashMap<String, ro.ds.tpt.model.Station>();
		for(ro.ds.tpt.model.Station s:c.getStations())
			stationExtIdMap.put(s.getExtId(), s);
	}
	
	private ro.ds.tpt.model.Station getStationByExtId(String extId) {
	...
	public void test_app_bus_panel_city_coverage() {
		String hiddenLines = "";
		for(ro.ds.tpt.model.Line l : c.getLines())
			if (!app_lines.contains(l.getName()))
				hiddenLines += "App does not show line "+l.getName()+"\n"; 
		assertEquals("", hiddenLines);
	}
	
	private static List<ro.ds.tpt.model.Station> stations(List<ro.ds.tpt.model.Estimate> estimates) {
		List<ro.ds.tpt.model.Station> st = new ArrayList<ro.ds.tpt.model.Station>();
		for (ro.ds.tpt.model.Estimate est : estimates) {
	...
	public void test_line_33_arta_connections() {
		ro.ds.tpt.model.Line l33 = c.getLineByName("33");
		ro.ds.tpt.model.Station a1 = getStationByExtId("2690");
		ro.ds.tpt.model.Station a2 = getStationByExtId("2669");
	...
	public void test_line_E1_arta_connections() {
		...
		ro.ds.tpt.model.Path p1 = lE1.getPath("Selgros");
		assertTrue(stations(p1.getEstimatesByPath()).contains(a1));

		ro.ds.tpt.model.Path p2 = lE1.getPath("Pod C. Sagului");
		assertTrue(stations(p2.getEstimatesByPath()).contains(a2));
	}
	...
	public void test_empty_junction() {
		for (ro.ds.tpt.model.Junction j:c.getJunctions())
	...
	public void test_Tv9() {
		ro.ds.tpt.model.Line tv9 = c.getLineByName("Tv9");
		assertTrue(tv9.getKind().isTram());
		assertTrue(tv9.getPaths().size()>0);
		for(ro.ds.tpt.model.Path p : tv9.getPaths())
	...
	public void assertLineInStation(String stationName, String lineName) {
		HashSet<ro.ds.tpt.model.Line> lines = new HashSet<ro.ds.tpt.model.Line>();
		ro.ds.tpt.model.Station st = getStationByExtId(stationName);
		for(ro.ds.tpt.model.Path p:st.getPaths())
				lines.add(p.getLine());
	...
	public void test_known_distance() {
		ro.ds.tpt.model.Station s33 = c.getLineByName("33").getPath("Pod C. Sagului").getEstimatesByPath().get(0).getStation();
	...
	public void test_known_distance_Arta_textila() {
		ro.ds.tpt.model.Station s33 = c.getLineByName("33").getPath("Pod C. Sagului").getEstimatesByPath().get(4).getStation();
		for(ro.ds.tpt.model.Station s : s33.getJunction().getStations()) {
			System.out.println(""+s.getId()+":"+s.getNiceName());
		}
	}
	

	public static void printDistance(ro.ds.tpt.model.Station a, ro.ds.tpt.model.Station b) {
		System.out.println("Distance: "+(long)a.distanceTo(b) + "m");
		System.out.println("    "+a.getId()+":"+a.getNiceName()+" - "+a.getLat()+"-"+a.getLng());
		for(ro.ds.tpt.model.Path p:a.getPaths())
			System.out.println("\t"+p.getId()+":"+p.getLineName());
		System.out.println("    "+b.getId()+":"+b.getNiceName()+" - "+b.getLat()+"-"+b.getLng());
		for(ro.ds.tpt.model.Path p:b.getPaths())
			System.out.println("\t"+p.getId()+":"+p.getLineName());	
	}
	...
	public void test_junction_stations_too_far() {
		...
		for(ro.ds.tpt.model.Junction j:c.getJunctions())
			for(ro.ds.tpt.model.Station a:j.getStations()) {
				if(a.getLat().trim().isEmpty() || a.getLng().trim().isEmpty()) continue;				
				for(ro.ds.tpt.model.Station b:j.getStations()) {
	...
	public void test_junction_stations_too_near() {
		...
		String e225="";		
		String e300="";		
		for(ro.ds.tpt.model.Station a:c.getStations())
			for(ro.ds.tpt.model.Station b:c.getStations()) {
				if (a==b) continue;
