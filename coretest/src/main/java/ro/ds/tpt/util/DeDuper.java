...
public class DeDuper {
	...
	private static class SimpleStation extends ro.ds.tpt.model.Station {
		private static final long serialVersionUID = 1L;
	...
	public static void main(String[] args) throws Exception {
		...
		String fileName = "Lines Stations and Junctions - Timisoara Public Transport - Denumiri-20160703.csv";
		...
		for(ro.ds.tpt.model.Station a:stMap.values())
			for(ro.ds.tpt.model.Station b:stMap.values()) {
				if (a==b) continue;
	...
	private static void selectJunctionMerger(ro.ds.tpt.model.Station a, ro.ds.tpt.model.Station b, int dist) {
		String message = 
				"The following junctions will be merged because they have stations that are near each-other.\n";
		List<ro.ds.tpt.model.Station> stations = new ArrayList<ro.ds.tpt.model.Station>();
		...
		for(int i=0;i<opts.size();i++) {
			...
			if (junction.getStations().size()<10) 
				...
				for(ro.ds.tpt.model.Station s:junction.getStations()) {
					junctionStations+="\n    "+s.getId()+"-"+s.getName()+"-"+s.getNiceName();
				}
			else
		...
		for(ro.ds.tpt.model.Station s:merged.getStations())
			s.setJunction(merged);
	}
	
	private static void selectCoords(String msg, ro.ds.tpt.model.Station st, Helper help) {
