...
import ro.ds.tpt.data.Achievements;
...
public class AppPreferences implements IPrefs {
	...
	public AppPreferences(Context ctx) {
		this.ctx = ctx;
		pref_device_id = ctx.getString(ro.ds.tpt.R.string.pref_device_id);
		pref_base_download_url = ctx.getString(ro.ds.tpt.R.string.pref_base_download_url);
		pref_analytics_enabled = ctx.getString(ro.ds.tpt.R.string.pref_analytics_enabled);
		pref_analytics_cache = ctx.getString(ro.ds.tpt.R.string.pref_analytics_cache);
		pref_current_version = ctx.getString(ro.ds.tpt.R.string.pref_current_version);
		pref_favorite_paths = ctx.getString(ro.ds.tpt.R.string.pref_favorite_paths);
		pref_achieve_name = ctx.getString(ro.ds.tpt.R.string.pref_achieve_name);
	...
	private String readBaseDownloadUrl(Context ctx) {
		...
		String defaultUrl = ctx.getString(ro.ds.tpt.R.string.pref_base_download_url_default);
	...
	public synchronized IAnalyticsService getAnalyticsService() {
		if (service == null) {
			...
			if (sharedPref.getBoolean(pref_analytics_enabled, true)) {
				...
				service = new ro.ds.tpt.analytics.AnalyticsService(ctx.getString(ro.ds.tpt.R.string.pref_analytics_url), agent);
			} else {
				service = new ro.ds.tpt.analytics.NoAnalyticsService();
			}
		}
		return service;
	}
	...
	public void addFavoritePath(Path path) {
		SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(ctx);
		ro.ds.util.PathList favorites = new ro.ds.util.PathList(sharedPref.getString(pref_favorite_paths, ""));
		sharedPref.edit()
	...
	public List<Path> getFavoritePaths(ro.ds.tpt.model.City c, int max) {
		SharedPreferences sharedPref = PreferenceManager.getDefaultSharedPreferences(ctx);
		ro.ds.util.PathList favorites = new ro.ds.util.PathList(sharedPref.getString(pref_favorite_paths, ""));
    	...
    	MOSTUSED: for (String line : LineKindAndroidEx.MOST_USED) {
    		for(Path p : favoritePaths)
    			if (p.getLineName().equals(line))
    				continue MOSTUSED;
    		ro.ds.tpt.model.Line cityLine = c.getLineByName(line);
    		if (cityLine == null)
    			continue;
